...
class sales_model extends CI_Model {
	...
	public function delete1($id,$data){
		
		$this->db->delete();
		$this->db->from("sales_transaction");
		$this->db->where('id',$id,$data);
	...
	public function showdatadb(){
		//$q = 'SELECT * FROM sales_transaction where status = "Deleted" ';
	...
	$sql = "SELECT customer.customer_name, COUNT(sales_transaction.customer_id) AS NumberOfOrders FROM sales_transaction LEFT JOIN customer ON sales_transaction.customer_id = customer.id GROUP BY customer_name ORDER BY COUNT(customer_id) DESC LIMIT 5";
