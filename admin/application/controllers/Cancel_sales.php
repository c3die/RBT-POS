...
class Cancel_sales extends MY_Controller {
	...
	function index(){
		if(isset($_GET['search'])){
			...
			if(!empty($_GET['id']) && $_GET['id'] != ''){
				$filter['cancelsales.id LIKE'] = "%".$_GET['id']."%";
			}

			if(!empty($_GET['date_from']) && $_GET['date_from'] != ''){
				$filter['DATE(cancelsales.date) >='] = $_GET['date_from'];
			}

			if(!empty($_GET['date_end']) && $_GET['date_end'] != ''){
				$filter['DATE(cancelsales.date) <='] = $_GET['date_end'];
			}

			$total_row = $this->cancel_sales_model->count_total_filter($filter);
			$data['sales'] = $this->cancel_sales_model->get_filter($filter,url_param());
		}else{
			$total_row = $this->cancel_sales_model->count_total();
			$data['sales'] = $this->cancel_sales_model->get_all();
		}
		$data['paggination'] = get_paggination($total_row,get_search());
		$this->load->view('sales/cancel_sales',$data);
	}
	...
	public function add_process(){
		...
		if($this->form_validation->run() != FALSE && !empty($carts) && is_array($carts)){
			...
			if($data['is_cash'] == 0){
				$data['pay_deadline_date'] = date('Y-m-d', strtotime("+15 days"));
			}else{
	...
	public function export_csv(){
		...
		$result = $this->cancel_sales_model->get_filter_csv($filter);
		if($result){
			$result = $this->_set_csv_format($result);
		}
		//echo json_encode($result);
		$this->csv_library->export('cancelsales.csv',$result);
	}
	public function print_now($id = ""){
		$details = $this->cancel_sales_model->get_detail($id);
		if($details){
			$data['details'] = $details;
