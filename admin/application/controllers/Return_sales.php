...
class return_sales extends MY_Controller {
	...
	function index(){
		if(isset($_GET['search'])){
			...
			$total_row = $this->sales->count_total_filter($filter);
			$data['sales'] = $this->sales->get_filter($filter,url_param());
		}else{
			$total_row = $this->sales->count_total();
			$data['sales'] = $this->sales->get_all(url_param());
		}
	...
	function create(){
		if(isset($_GET['search'])){
			...
			$data['sales'] = $this->sales_model->get_all(url_param());
		}
		$data['return'] = true;
		$data['paggination'] = get_paggination($total_row,get_search());
		$this->load->view('return_sales/return_index',$data);
	}
	...
	function create_return($id){
		...
		$data['code_sales'] = $id;
		$data['code_return_sales'] = "RETS".strtotime(date("Y-m-d H:i:s"));
		$data['customers'] = $this->customer_model->get_all();
		$data['categories'] = $this->category_model->get_all();
		$data['details'] = $details;
		$this->load->view('return_sales/form',$data);
	}
	
	public function detail($id){
		$details = $this->sales->get_detail_by_id($id);
		//print_r($details); exit;
		if($details){
			$data['details'] = $details;
			$this->load->view('return_sales/detail',$data);
		}else{
			redirect(site_url('return_sales'));
		}
	}
	...
	public function edit($return_id){
		...
		$details = $this->sales->get_detail($return_id);
		...
		$data['edit'] = true;
		$data['carts'] = $cart_data;
		$data['code_sales'] = $details[0]->sales_id;
	...
	public function update($return_id){
		$details = $this->sales->get_detail_by_id($return_id);
		if(!$details){
			redirect(site_url());
			exit;
		}
		...
		if(!empty($carts) && is_array($carts) && $check_qty){
			...
			foreach($details as $detail){
				$this->sales->delete_data($detail->sales_id);
			}
			...
			$this->sales->update($return_id,$data);
			if($is_return == "1" && $is_return_old != 1){
				...
				// purchase_return
				$return_p['id'] = "RETS".strtotime(date("Y-m-d H:i:s"));
				$return_p['sales_return_id'] = $return_id;
				$return_p['total_price'] = $this->cart->total();
				$return_p['total_item'] = $this->cart->total_items();
				$return_p['is_return'] = "0";
				$return_p['return_by'] = "1";
				$this->return_purchase->insert($return_p);

				$this->return_purchase->insert_return_carts($return_p['id'],$carts);
			}
	...
	public function export_csv(){
		$data = $this->sales->get_filter('',url_param(),true);
		$this->csv_library->export('salesreturn.csv',$data);
	}
}
